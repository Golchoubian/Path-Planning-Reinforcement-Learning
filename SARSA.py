# -*- coding: utf-8 -*-
"""
Created on Fri Nov  6 17:23:13 2020
...
def SARSA_alg(env , x_start, x_goal, gamma, alpha, epsilon, max_Iter, max_step, wall_penalty, goal_state_reward):
    ...
    while (Iter <= max_Iter):# and Q_change>0.0001):
        ...
        while True:
            ...
            if (x_tran[0]<0 or x_tran[0]>=np.shape(env)[0] or x_tran[1]<0 or x_tran[1]>=np.shape(env)[1] ):
                ...
                x_new = x_tran
                if (x_new == x_goal).all():
                   r = goal_state_reward # this was 20 before! # The goal is reached!
                   GoalReached = True
                   Q_new = r
                else:
                   r = -1 # to encourage getting to the goal soon
                   
                   # Select an action at the new satet according to the behaviour policy which is an epsilon-greedy policy
                   rand_n2 = random.random()
                   if rand_n2 <= epsilon:
                      # Then choose a randon action for exploration
                      next_act_index = random.randint(0,3) 
                   else:     
                      next_act_index = np.argmax(Q_table[:,x_new[0],x_new[1]])
                   
                   # The next line is the difference between the SARSA and the Q_learning algorithm
                   Q_new = r + gamma*Q_table[next_act_index,x_new[0],x_new[1]]
